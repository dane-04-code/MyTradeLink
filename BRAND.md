...
**TradeLink** is a smart digital profile page builder for UK tradesmen — Linktree built for plumbers, not influencers.
...
- No emojis in body copy (only in selected callouts like "Your TradeLink page is live 🎉").
...
### Type rules
- Display font (Archivo Black) is ALWAYS used for: page headlines, big numbers (e.g. analytics stats, pricing), the TRADELINK wordmark, business names on profiles.
...
These are the specific aesthetic moves that make a TradeLink visual recognisable. Use them.
...
### 5.5 Phone mockup
A 310×640 dark ink-900 phone shape with a notch indicator, rounded 44px corners, drawn in CSS (not a stock photo). Used on the landing hero to show a real-looking finished TradeLink page inside. Avoid stock-photo phones.
...
**TRADELINK** wordmark — always set in Archivo Black, uppercase, slight negative tracking (`-0.02em`).
...
1. **Customer outcomes** — tradesman screenshots a great quote request that came in via their TradeLink page
