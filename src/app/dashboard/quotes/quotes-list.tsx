...
export function QuotesList({ quotes }: { quotes: QuoteRequest[] }) {
  ...
  if (items.length === 0) {
    return (
      <div className="rounded-xl border-2 border-dashed border-ink-900/30 bg-white p-12 text-center">
        <div className="mx-auto flex h-14 w-14 items-center justify-center rounded-lg border-2 border-ink-900 bg-ink-900 text-brand">
...
function Filters({
  ...
  return (
    <div className="mb-4 flex overflow-x-auto rounded-lg border-2 border-ink-900 bg-white p-1">
      {tabs.map((t) => {
        ...
        return (
          <button
            ...
            className={cn(
              "inline-flex flex-1 items-center justify-center gap-2 whitespace-nowrap rounded-md px-3 py-2 text-xs font-bold transition",
    ...
    >
      ...
      <div className="flex flex-wrap items-start justify-between gap-3 p-5 pb-3">
          ...
          <div className="mt-1.5 flex flex-wrap items-center gap-x-4 gap-y-1 text-sm text-ink-700">
            ...
            {q.customerEmail && (
              <a
                ...
                className="inline-flex min-w-0 items-center gap-1.5 truncate font-bold hover:text-brand"
              >
                <Mail className="h-3.5 w-3.5 flex-shrink-0" />
                <span className="truncate">{q.customerEmail}</span>
              </a>
            )}
            {q.postcode && (
        ...
        <button
          ...
          className="inline-flex flex-shrink-0 items-center gap-1.5 rounded-md border-2 border-line bg-white px-2.5 py-1.5 text-[11px] font-bold text-ink-700 transition hover:border-ink-900 hover:text-ink-900"
        >
      ...
      <div className="px-5 pb-4">
        <div className="rounded-lg border-2 border-line bg-muted p-4 text-sm leading-relaxed text-ink-900 whitespace-pre-line">
          {q.jobDescription}
        </div>
            ...
            {photoUrls.map((u) => (
              ...
              <a key={u} href={u} target="_blank" rel="noopener noreferrer">
                <img
                  src={u}
                  alt=""
                  className="h-20 w-20 flex-shrink-0 rounded-md border-2 border-ink-900 object-cover"
                />
              </a>
            ))}
          </div>
      ...
      {/* Actions */}
      <div className="flex flex-wrap items-center gap-2 border-t-2 border-ink-900 bg-muted/40 px-5 py-3">
        <a
          href={`tel:${q.customerPhone}`}
          className="inline-flex items-center gap-1.5 rounded-md border-2 border-ink-900 bg-call px-3 py-2 text-xs font-bold text-white shadow-hard-sm transition active:translate-y-0.5 active:shadow-press"
        >
        ...
        <a
          ...
          className="inline-flex items-center gap-1.5 rounded-md border-2 border-ink-900 bg-whatsapp px-3 py-2 text-xs font-bold text-white shadow-hard-sm transition active:translate-y-0.5 active:shadow-press"
        >
        ...
        <div className="ml-auto flex gap-2">
          {q.status !== "contacted" && (
            <button
              ...
              className="inline-flex items-center gap-1 rounded-md border-2 border-ink-900 bg-white px-3 py-2 text-xs font-bold text-ink-900 transition active:translate-y-0.5 hover:bg-muted"
            >
              <Check className="h-3 w-3" strokeWidth={3} /> Mark contacted
            </button>
          )}
          {q.status !== "closed" && (
            <button
              ...
              className="rounded-md border-2 border-ink-900 bg-ink-900 px-3 py-2 text-xs font-bold text-white transition active:translate-y-0.5 hover:bg-ink-800"
            >
              Close
            </button>
          )}
          {q.status === "closed" && (
            <button
              ...
              className="rounded-md border-2 border-line bg-white px-3 py-2 text-xs font-bold text-ink-700 transition hover:border-ink-900 hover:text-ink-900"
            >
              Reopen
            </button>
          )}
...
function StatusBadge({ status }: { status: QuoteRequest["status"] }) {
  ...
  if (status === "contacted") {
    return (
      <span className="inline-flex items-center gap-1 rounded-sm border border-call bg-white px-1.5 py-0.5 text-[10px] font-bold uppercase tracking-[0.18em] text-call">
        <Check className="h-2.5 w-2.5" strokeWidth={3} /> Contacted
      </span>
    );
  }
  return (
    <span className="rounded-sm border border-ink-500/40 bg-muted px-1.5 py-0.5 text-[10px] font-bold uppercase tracking-[0.18em] text-ink-500">
      Closed
    </span>
  );
}
