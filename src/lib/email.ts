...
export async function sendWelcomeEmail(to: string, name: string) {
  ...
  await resend.emails.send({
    from: FROM,
    to,
    subject: "Welcome to TradeLink — your page is ready",
    html: `
      <div style="font-family:Inter,system-ui,sans-serif;max-width:560px;margin:0 auto;padding:24px;color:#0B0B0F">
        <h1 style="color:#FF6B00;margin:0 0 12px">Welcome, ${escape(name)}.</h1>
        <p style="font-size:16px;line-height:1.5">Your professional trade page is ready. Finish setting it up — it takes 5 minutes.</p>
        <p style="margin-top:24px"><a href="${appUrl}/onboarding" style="display:inline-block;background:#FF6B00;color:#fff;padding:14px 22px;border-radius:14px;text-decoration:none;font-weight:700">Finish your page</a></p>
...
export async function sendNewQuoteEmail(opts: {
  ...
  await resend.emails.send({
    from: FROM,
    to: opts.to,
    subject: `New job: ${opts.customerName}${opts.postcode ? ` in ${opts.postcode}` : ""}`,
    html: `
      <div style="font-family:Inter,system-ui,sans-serif;max-width:560px;margin:0 auto;padding:24px;color:#0B0B0F">
        <h1 style="color:#FF6B00;margin:0 0 8px">New quote request</h1>
        ...
        <p style="margin-top:20px"><a href="${appUrl}/dashboard/quotes" style="display:inline-block;background:#FF6B00;color:#fff;padding:12px 18px;border-radius:14px;text-decoration:none;font-weight:700">View in dashboard</a></p>
      </div>
    `,
  });
}
